...
describe('TravelPreferencesTab - First Time User Experience', () => {
  ...
  it('allows budget slider to work for first-time users', async () => {
    render(<TravelPreferencesTab />);
    
    // Find the budget slider (it has multiple range inputs)
    const sliders = screen.getAllByRole('slider');
    const budgetSlider = sliders.find(slider => 
      slider.getAttribute('min') === '0' && slider.getAttribute('max') === '50000'
    );
    
    expect(budgetSlider).toBeInTheDocument();
    
    // Simulate changing the budget
    if (budgetSlider) {
      fireEvent.change(budgetSlider, { target: { value: '2000' } });
  ...
  it('shows profile selector after first profile is created', async () => {
    ...
    const { rerender } = render(<TravelPreferencesTab />);
