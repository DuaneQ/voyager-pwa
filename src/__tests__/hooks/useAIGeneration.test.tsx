...
describe('useAIGeneration hook', () => {
    ...
    mockHttpsCallable.mockImplementation((functions: any, name: string) => {
      if (name === 'generateItineraryWithAI') {
        return jest.fn(() => Promise.resolve({ data: {
          id: 'server-gen-1',
          response: { success: true, data: { itinerary: { id: 'server-gen-1' }, recommendations: { transportation: {
            mode: 'train', estimatedTime: '6h', estimatedDistance: '400km', estimatedCost: { amount: 60, currency: 'EUR' }, provider: 'Deutsche Bahn', tips: 'Book early for best prices.'
          } } } }
        } }));
      }
  ...
  expect(savedArg.response.data.recommendations.transportation.mode).toBe('train');
  expect(savedArg.response.data.recommendations.transportation.provider).toBe('Deutsche Bahn');
  });

  it('calls searchFlights when profile primaryMode is airplane', async () => {
    const mockHttpsCallable = mockedFunctions.httpsCallable as jest.Mock;
    const searchFlightsMock = jest.fn(() => Promise.resolve({ data: { flights: [{ id: 'f1' }] } }));
  const generateItineraryWithAIMock = jest.fn(() => Promise.resolve({ data: { id: 'server-gen-air', response: { success: true, data: { itinerary: { id: 'server-gen-air' }, recommendations: { flights: [{ id: 'f1' }] } } } } }));

    mockHttpsCallable.mockImplementation((functions: any, name: string) => {
      if (name === 'searchFlights') return searchFlightsMock;
      if (name === 'searchAccommodations') return jest.fn(() => Promise.resolve({ data: { hotels: [] } }));
      if (name === 'searchActivities') return jest.fn(() => Promise.resolve({ data: { activities: [] } }));
      if (name === 'generateItineraryWithAI') return generateItineraryWithAIMock;
      return jest.fn(() => Promise.resolve({ data: {} }));
    });
  ...
  const generateItineraryWithAIMock = jest.fn(() => Promise.resolve({ data: { id: 'server-gen-2', response: { success: true, data: { itinerary: { id: 'server-gen-2' }, recommendations: { transportation: { mode: 'bus' } } } } } }));
        ...
        if (name === 'generateItineraryWithAI') {
          ...
          return jest.fn(() => Promise.resolve({ data: {
            ...
            response: { success: true, data: {
              ...
              recommendations: {
                alternativeActivities: [ { id: 'a1', name: 'Louvre Tour' }, { id: 'a2', name: 'Eiffel Visit' } ],
  ...
  it('handles activities callable rejection gracefully and still saves generation with empty activities', async () => {
    ...
    mockHttpsCallable.mockImplementation((functions: any, name: string) => {
      ...
      if (name === 'generateItineraryWithAI') return jest.fn(() => Promise.resolve({ data: {
        id: 'server-gen-places-failed', response: { success: true, data: { itinerary: { id: 'server-gen-places-failed' }, recommendations: { alternativeActivities: [], alternativeRestaurants: [], flights: [], accommodations: [] } } }
      } }));
      return jest.fn(() => Promise.resolve({ data: {} }));
    });
  ...
  it('parses nested activities response shapes correctly', async () => {
    ...
    mockHttpsCallable.mockImplementation((functions: any, name: string) => {
      ...
      if (name === 'generateItineraryWithAI') return jest.fn(() => Promise.resolve({ data: {
        id: 'server-gen-nested', response: { success: true, data: { itinerary: { id: 'server-gen-nested' }, recommendations: { alternativeActivities: [{ id: 'a2', name: 'Nested Tour' }], alternativeRestaurants: [], flights: [], accommodations: [] } } }
      } }));
      return jest.fn(() => Promise.resolve({ data: {} }));
    });
    ...
    expect(Array.isArray(savedArg.response.data.recommendations.alternativeActivities)).toBe(true);
    expect(savedArg.response.data.recommendations.alternativeActivities?.length).toBeGreaterThanOrEqual(0);
  });
