...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  React.useEffect(() => {
    ...
    if (isPlaying && hasUserInteracted) {
      ...
      console.log(
        "VideoPlayer: Not playing because hasUserInteracted is false"
      );
    }
    ...
    <div className={`video-player ${className}`} data-testid="video-player">
      ...
      {showInfoOverlay && (
        <div className="video-info-overlay">
          {video.title && (
            <h3 className="video-title" data-testid="video-title">
              {video.title}
            </h3>
          )}
          {video.description && (
            <p className="video-description" data-testid="video-description">
              {video.description}
            </p>
          )}
        </div>
